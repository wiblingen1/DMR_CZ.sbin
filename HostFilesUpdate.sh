...
curl -sSL http://barrandovhblink.jednoduse.cz/dmrcz/DMRIds.dat.gz --user-agent "Pi-Star_${pistarCurVersion}" | gunzip -c > ${DMRIDFILE}
...
if [ -f ${NEXTIONGROUPS} ]; then
	# Update ${NEXTIONGROUPS}
 	if [[ $(find "${NEXTIONGROUPS}" -mtime +7) ]]; then
  		curl --fail -o ${NEXTIONGROUPS} -s http://barrandovhblink.jednoduse.cz/dmrcz/groups.txt --user-agent "Pi-Star_${pistarCurVersion}"
  	fi
else
	# Get ${NEXTIONGROUPS}
 	curl --fail -o ${NEXTIONGROUPS} -s http://barrandovhblink.jednoduse.cz/dmrcz/groups.txt --user-agent "Pi-Star_${pistarCurVersion}"
